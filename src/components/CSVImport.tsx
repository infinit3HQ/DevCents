...
function detectColumnMapping(headers: string[]): ColumnMapping {
  ...
  return {
    date: Math.max(
      0,
      lower.findIndex((h) => ["date", "time", "timestamp", "when"].includes(h)),
    ),
    ...
    amount: Math.max(
      0,
      lower.findIndex((h) =>
        ["amount", "value", "sum", "total", "price"].includes(h),
      ),
    ),
    type: lower.findIndex((h) =>
      ["type", "kind", "direction", "credit/debit"].includes(h),
    ),
  };
}
...
function parseRow(row: string[], mapping: ColumnMapping): ParsedRow {
  try {
    ...
    const typeStr =
      mapping.type >= 0 ? (row[mapping.type] || "").toLowerCase() : "";
...
export function CSVImport({ trigger }: { trigger?: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    return () =>
      document.removeEventListener("open-csv-import", handleOpenCSVImport);
  }, []);
      ...
      const transactions = await Promise.all(
        validRows.map(async (r) => ({
          amount: shouldEncrypt
            ? await encryptValue(String(r.amount))
            : r.amount,
          type: r.type,
          category: r.category,
          description: shouldEncrypt
            ? await encryptValue(r.description)
            : r.description,
  ...
  return (
    ...
    >
      <SheetTrigger asChild>
        {trigger || (
          <Button
            variant="outline"
            size="sm"
            className="gap-2 rounded-none border-2 border-border font-mono uppercase tracking-widest text-xs hover:bg-foreground hover:text-background transition-none"
          >
      ...
      <SheetContent
        side="right"
        className="w-full sm:max-w-xl overflow-y-auto border-l-4 border-border/10 bg-background/95 backdrop-blur-xl"
      >
        <SheetHeader className="border-b-2 border-border pb-6 mb-8 text-left">
          <SheetTitle className="font-serif text-4xl tracking-tight">
            Import Records
          </SheetTitle>
          <SheetDescription className="font-mono text-xs uppercase tracking-widest text-muted-foreground">
            Batched transaction ingestion protocol.
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="space-y-8">
          {!done && (
            ...
            >
              <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-border cursor-pointer hover:bg-foreground hover:text-background transition-colors group">
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  ...
                  <p className="text-sm font-mono uppercase tracking-widest">
                    {fileName ? (
                      ...
                      <span>
                        Select{" "}
                        <span className="text-primary group-hover:text-background font-bold">
                          .csv
                        </span>{" "}
                        file
                      </span>
          ...
          {headers.length > 0 && !done && (
            <motion.div
              ...
              className="space-y-4"
            >
              <h4 className="text-xs font-bold text-foreground font-mono uppercase tracking-widest border-b border-border pb-2">
                Column Mapping
              </h4>
                ...
                {EXPECTED_HEADERS.map((field) => (
                  <div key={field} className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        updateMapping(field as keyof ColumnMapping, parseInt(v))
                      }
                    >
          ...
          {rows.length > 0 && !done && (
            ...
            >
              <div className="flex items-end justify-between border-b border-border pb-2">
                <h4 className="text-xs font-bold text-foreground font-mono uppercase tracking-widest">
                  Preview
                </h4>
                    ...
                    <AnimatePresence>
                      {rows.slice(0, 50).map((row, i) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-3 truncate max-w-[120px] font-serif tracking-tight">
                            {row.description}
                          </td>
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs font-mono uppercase tracking-widest text-muted-foreground">
                  {validCount} records committed to ledger.
                </p>
              </div>
              <div className="flex gap-4 mt-4">
                <Button
                  variant="outline"
                  onClick={reset}
                  className="rounded-none border-2 border-border font-mono uppercase tracking-widest text-xs px-6"
                >
                  New Batch
                </Button>
                <Button
                  onClick={() => setOpen(false)}
                  className="rounded-none bg-foreground text-background hover:bg-foreground/90 font-mono uppercase tracking-widest text-xs px-6"
                >
                  Close
                </Button>
              </div>
